...
export default function BlocoBScreen({ navigation }) {
  const [salasB, setSalasB] = useState([]);
  ...
  async function getAllSalasB() {
    try {
      const response = await api.getAllSalasB();
      setSalasB(response.data.salas);
      setLoading(false);
    } catch (error) {
